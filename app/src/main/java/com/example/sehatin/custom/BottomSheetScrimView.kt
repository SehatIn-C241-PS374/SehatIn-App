...
/**
 * Scrim for background of the bottom sheet with the thumbnail after object successfully detected
 * */
class BottomSheetScrimView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    fun updateWithThumbnailTranslateAndScale(
        ...
        // Update the Rect Dynamically
        thumbnailRect = RectF().apply {
            val destX = thumbnailMargin.toFloat()
            val destY = (height - collapsedStateHeight - thumbnailMargin - thumbnailHeight).toFloat()
            val destHeight = thumbnailHeight.toFloat()
            val destWidth = srcThumbnailRect.width() / srcThumbnailRect.height() * destHeight
            val dstRect = RectF(destX, destY, destX + destWidth, destY + destHeight)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (bitmap != null && rect != null) {
            // give an entire view a scrim black paint
